...
def check_elem_action_seq(warehouse, action_seq):
    ...
    # copies warehouse into a new warehouse puzzle
    new_warehouse = warehouse.copy(worker, boxes)

    for action in action_seq:
        # Check if the move is possible first
        # If it is not possible, return 'Failure'
        if check_move(new_warehouse, action):
            ...
            for coordinate, box in enumerate(boxes):
                if box == (worker[0], worker[1]):
                    box_x = box[0] + x
                    box_y = box[1] + y
                    boxes[coordinate] = (box_x, box_y)
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
def check_move(warehouse, action):
    ...
    cell1 = worker_x + x1, worker_y + y1
    cell2 = worker_x + x2, worker_y + y2
    if cell1 in walls:
        return False
    elif cell1 in boxes and cell2 in boxes:
        return False
    elif cell1 in boxes and cell2 in walls:
        return False
    else:
        return True
